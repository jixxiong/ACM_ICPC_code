...
int32_t main() {
    ...
    while (T--) {
        i64 S, C; std::cin >> S >> C;
        auto cal = [&](i64 x) -> i64 {
            long long ans = 1ll * (S + S - x + 2) * (x - 1) / 2;
            int cur = S - x + 1;
            return ans + 1ll * C * (cur % (C + 1) + cur) * (cur / (C + 1) + 1) / 2;
        };
        i64 lm = std::max(1ll, S + 1 - C * (C + 1) / 2);
        i64 rm = std::max(1ll, std::min(S, S + C + 1 - C * (C + 1) / 2));
        i64 ans = 0;
        for (i64 x = lm; x <= rm; ++x) {
            ans = std::max(ans, cal(x));
        }
        std::cout << ans << '\n';
    }
    return 0;
