...
int main(){
    ...
    s=' '+s;
    SA sa(&s[0],n);
	vector<int>B(&sa.ht[1],&sa.ht[n]+1);
	list<int>s1{0};
	ll sum=0,ret=(ll)n*(n-1)*(n+1)/2;
	for(int i=1;i<=n-1;++i){
		while(s1.back()&&B[i]<=B[s1.back()])
			sum-=(ll)(s1.back()-*next(s1.rbegin()))*B[s1.back()],s1.pop_back();
		s1.push_back(i);
		sum+=(ll)(s1.back()-*next(s1.rbegin()))*B[s1.back()];
		ret-=2*sum;
	}
	cout<<ret<<'\n';
    return 0; 
