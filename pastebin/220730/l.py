...
sqrt2 = dcm(sqrt(2))
sqrt5 = dcm(sqrt(5))
fuck = dcm(sqrt((1/2)+1/(2*sqrt(5)))) / dcm(tan(PI/5))
...
for _ in range(int(input())):
    ...
    a=dcm(a)
    if not (n==4 or n==6 or n==8 or n==12 or n==20):
        print('impossible')
    else:
        while k>0:
            n,a=change(n,a)
            k-=1
        print(f'possible {n} {a:.10f}')
...
# ld const fuck=std::sqrt((1/(ld)2)+1/(2*sqrt5)) / (std::tan(5/PI));
